#!/usr/bin/env node
// Generated from packages/buddy/src/skills/min-release-age.ts by `pnpm build` — do not edit.
/*
...
function remoteHost(url) {
	return /^(?:[a-z+]+:\/\/)?(?:[^@/]+@)?([^/:]+)/i.exec(url ?? "")?.[1]?.toLowerCase() ?? null;
}
...
function detectCi(dir, remote) {
	let url = remote;
	if (url === void 0) try {
		url = execFileSync("git", [
			"-C",
			dir,
			"remote",
			"get-url",
	...
	const systems = [...Object.keys(PROVIDERS).filter((p) => PROVIDERS[p]?.detect.some((f) => existsSync(join(dir, f)))), ...Object.keys(OTHER_CI).filter((p) => existsSync(join(dir, OTHER_CI[p] ?? "")))];
	if (["forgejo", "gitea"].includes(host) && systems.includes("github") && !systems.includes(host)) systems.splice(systems.indexOf("github"), 1, host);
	const installed = Object.keys(PROVIDERS).find((p) => hasJob(dir, p)) ?? null;
	const provider = installed ?? (systems.includes(host) ? host : void 0) ?? systems[0] ?? (PROVIDERS[host] ? host : "other");
...
//#endregion
//#region src/release-age/managers.ts
const MANAGERS = {
	pnpm: {
...
function detectManager(dir) {
	...
	if (existsSync(pkgPath)) {
		...
		const name = typeof field === "string" ? field.split("@")[0] : void 0;
	...
	if (existsSync(join(dir, "bun.lock")) || existsSync(join(dir, "bun.lockb")) || existsSync(join(dir, "bunfig.toml"))) return "bun";
	if (existsSync(join(dir, "package-lock.json")) || existsSync(join(dir, ".npmrc"))) return "npm";
}

//#endregion
//#region src/release-age/config-file.ts
...
function readAge(pm, lines) {
	...
	const re = format === "ini" ? new RegExp(`^\\s*${ageKey}\\s*=\\s*(.+?)\\s*$`) : format === "toml" ? new RegExp(`^\\s*${ageKey}\\s*=\\s*(.+?)\\s*(#.*)?$`) : new RegExp(`^${ageKey}:\\s*(.+?)\\s*(#.*)?$`);
	for (const line of format === "toml" ? sectionLines(lines, "install") : lines) {
		const m = re.exec(line);
		if (m?.[1] !== void 0) return unquote(m[1]);
	}
}
...
function readExemptions(pm, lines) {
	...
	if (format === "ini") {
		...
		lines.forEach((line, i) => {
			const m = re.exec(line);
			if (m?.[2] !== void 0) push(unquote(m[2]), i);
		});
		return entries;
	}
	if (format === "yaml") {
		...
		for (let i = block.start + 1; i < block.end; i++) {
			...
			if (m?.[1] !== void 0) push(unquote(m[1]), i);
		}
		for (const inline of block.inline) entries.push({
			value: inline,
			line: block.start + 1,
			until: null
		});
	...
	if (arr.start === arr.end) {
		for (const v of arr.inline) entries.push({
			value: v,
			line: arr.start + 1,
			until: null
		});
...
function yamlBlock(lines, key) {
	...
	const rest = (lines[start] ?? "").slice(key.length + 1).replace(/#.*$/, "").trim();
...
function insertEntry(pm, lines, entry, markerText) {
	const { excludeKey, format } = MANAGERS[pm];
	if (format === "ini") {
		for (let i = 0; i < lines.length; i++) lines[i] = (lines[i] ?? "").replace(new RegExp(`^(\\s*${excludeKey})\\s*=`), "$1[]=");
	...
	if (arr.start === arr.end) {
		...
		lines.splice(arr.start, 1, `${arr.indent}${excludeKey} = [`, ...items, `${inner}# ${markerText}`, `${inner}"${entry}",`, `${arr.indent}]`);
		return;
	}
...
//#endregion
//#region src/release-age/core.ts
function status(dir, pm, now = /* @__PURE__ */ new Date()) {
	...
	return {
		...
		value: raw ?? null,
		unit: cfg.unit,
		minutes: raw === void 0 ? cfg.defaultMinutes : toMinutes(pm, raw) ?? null,
		defaultApplied: raw === void 0,
		defaultNote: raw === void 0 ? cfg.defaultNote : void 0,
...
function npmView(name, field) {
	const parsed = JSON.parse(execFileSync("npm", [
		"view",
		name,
		field,
		"--json"
...
function resolveSpec(spec, registry = npmRegistry) {
	const { name, version } = splitSpec(spec);
	if (!name || name.startsWith(".") || name.includes(" ")) return { error: `invalid package "${spec}"` };
	if (version && EXACT.test(version)) return {
		name,
		version
	};
	if (version !== void 0 && !TAG.test(version)) return { error: `"${spec}" is a range; pass an exact version, a dist-tag, or the bare name for latest` };
...
function lift(dir, pm, input, options = {}) {
	const { nameWide = false, until, now = /* @__PURE__ */ new Date(), registry = npmRegistry } = options;
	const cfg = MANAGERS[pm];
	...
	if (!cfg.versionPin && !nameWide) return {
		ok: false,
	...
	const covered = [...current.permanent, ...current.lifts.map((l) => l.value)].find((v) => v === entry || v === parsed.name || v.endsWith("/*") && parsed.name.startsWith(v.slice(0, -1)));
	if (covered) return {
		ok: true,
		changed: false,
		entry,
		spec,
		reason: `already exempt by "${covered}"`
	};
	let expiry = until ? new Date(until) : void 0;
	if (!expiry) {
		if (!current.minutes) return {
			ok: false,
			error: `${cfg.ageKey} is not set; there is no gate to lift`
		};
		const published = registry.times(parsed.name)[parsed.version];
		if (!published) return {
			ok: false,
			error: `npm has no publish time for ${spec}`
		};
		expiry = new Date(new Date(published).getTime() + current.minutes * 6e4);
		expiry.setUTCMinutes(0, 0, 0);
		expiry = new Date(expiry.getTime() + 36e5);
	}
	if (Number.isNaN(expiry.getTime())) return {
		ok: false,
		error: `invalid expiry "${until}"`
	};
	if (expiry <= now) return {
		...
		entry,
		spec,
		reason: `${spec} already clears the gate; no lift needed`
	};
	...
	return {
		...
		entry,
		until: iso,
		nameWide: !cfg.versionPin
	};
}
function restore(dir, pm, options = {}) {
	const { now = /* @__PURE__ */ new Date(), dryRun = false } = options;
	const { excludeKey, format } = MANAGERS[pm];
	const { path, lines } = load(dir, pm);
	const removed = [];
	const kept = [];
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (!next.includes(name)) return {
			ok: false,
			error: `${path}:${i + 1}: marker for ${m[1]} is not followed by its entry`
		};
		removed.push({
			lift: m[1],
			until: m[2],
	...
	if (format === "yaml") {
		...
		if (block && !block.flow && !out.slice(block.start + 1, block.end).some((l) => /^\s+-\s/.test(l))) out.splice(block.start, block.end - block.start, `${excludeKey}: []`);
	}
...
//#endregion
//#region src/release-age/open-pr.ts
...
const PR_APIS = {
	bitbucket: async (env, { branch, body }, fetchFn) => {
		...
		const [open] = (await call(fetchFn, `${api}?q=${encodeURIComponent(`source.branch.name="${branch}" AND state="OPEN"`)}`, { headers })).values ?? [];
		if (open) return {
			action: "updated",
			url: (await call(fetchFn, `${api}/${open.id}`, {
				method: "PUT",
				headers,
				body: JSON.stringify({
					title: RESTORE_TITLE,
	...
	azure: async (env, { branch, body }, fetchFn) => {
		...
		const [open] = (await call(fetchFn, `${api}?${`searchCriteria.sourceRefName=${encodeURIComponent(source)}&searchCriteria.status=active`}&api-version=7.1`, { headers })).value ?? [];
		if (open) {
			...
			return {
				action: "updated",
				id: open.pullRequestId
			};
		}
		return {
			action: "created",
			id: (await call(fetchFn, `${api}?api-version=7.1`, {
				method: "POST",
				headers,
				body: JSON.stringify({
	...
	forgejo: async (env, { branch, body }, fetchFn) => {
		const [server, repo, base] = required(env, [
			"GITHUB_SERVER_URL",
			"GITHUB_REPOSITORY",
			"GITHUB_REF_NAME"
		]);
		...
		const open = (await call(fetchFn, `${api}?state=open&limit=50`, { headers })).find((p) => p.head?.ref === branch);
		if (open) return {
			action: "updated",
			url: (await call(fetchFn, `${api}/${open.number}`, {
				method: "PATCH",
				headers,
...
async function openPr(provider, args, env = process.env, fetchFn = fetch) {
	const api = PR_APIS[provider];
	if (!api) return {
		ok: false,
		error: `open-pr supports ${Object.keys(PR_APIS).join(", ")}; got "${provider}"`
	};
	try {
		return {
			ok: true,
			provider,
			...await api(env, args, fetchFn)
		};
	} catch (error) {
		return {
			ok: false,
...
//#endregion
//#region src/skills/min-release-age.ts
function usage(message) {
	process.stderr.write(`${message}\n`);
	process.stderr.write("usage: min-release-age.mjs <status|lift <pkg[@version|@tag]>|restore|open-pr> [--dir <repo>] [--pm <pm>] [--name-wide] [--until <ISO>] [--now <ISO>] [--dry-run] [--json] [--github-output] [--body-file <path>] [--provider <p>] [--branch <b>]\n");
	process.exit(2);
}
...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		if (VALUED.has(a)) {
			...
			if (value === void 0) usage(`${a} needs a value`);
...
async function main(argv = process.argv.slice(2)) {
	...
	if (!command || ![
		...
		"restore",
		"open-pr"
	].includes(command)) usage(`unknown command "${command ?? ""}"`);
	if (command === "open-pr") {
		if (typeof opts.provider !== "string" || typeof opts["body-file"] !== "string") usage("open-pr needs --provider and --body-file");
		...
		const result = await openPr(opts.provider, {
			branch,
			body
		});
		if (!result.ok) {
	...
	if (!pmName || !isPackageManager(pmName)) usage(`cannot detect the package manager in ${dir}; pass --pm pnpm|yarn|npm|bun`);
	const pm = pmName;
	...
	if (command === "lift") {
		...
		const liftOptions = {
			nameWide: Boolean(opts["name-wide"]),
			now
		};
		if (opts.until !== void 0) liftOptions.until = opts.until;
		try {
	...
	if (opts.json) process.stdout.write(`${JSON.stringify(result, null, 2)}\n`);
	else if ("changed" in result) process.stdout.write(result.changed ? `lifted ${result.spec}${result.tag ? ` (${result.tag})` : ""} as '${result.entry}' in ${result.file} until ${result.until}${result.nameWide ? " (whole package name)" : ""}\n` : `no change: ${result.reason}\n`);
	else if ("removed" in result) {
		const verb = result.dryRun ? "would remove" : "removed";
		process.stdout.write(`${verb} ${result.removed.length} expired lift(s)${result.removed.map((r) => `\n  ${r.lift} (until ${r.until})`).join("")}\n${result.kept.length} active lift(s) kept\n`);
	}
}
