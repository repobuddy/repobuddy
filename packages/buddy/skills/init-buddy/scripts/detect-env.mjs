#!/usr/bin/env node
// Generated from packages/buddy/src/skills/detect-env.ts by `pnpm build` — do not edit.
/*
...
function detectOs(env) {
	...
	} else if (plat === "win32") os.family = "windows";
	else if (plat === "linux") {
	...
	github: {
		...
		install: [
			...
			},
			{
				...
				os: ["windows"],
				official: false,
				commands: ["scoop install gh"]
			},
			{
				manager: "choco",
				os: ["windows"],
				official: false,
				commands: ["choco install gh"]
			},
			{
				manager: "apt-get",
				os: ["debian"],
			...
			},
			{
				...
				official: false,
				sudo: true,
				commands: ["sudo pacman -S github-cli"]
			},
			{
				manager: "apk",
				os: ["alpine"],
				official: false,
				sudo: true,
				commands: ["sudo apk add github-cli"]
			},
			{
				manager: "nix",
				os: ["macos", "linux"],
				official: false,
				commands: ["nix profile install nixpkgs#gh"]
			},
			{
				manager: "conda",
				os: [
	...
	gitlab: {
		...
		install: [
			...
			},
			{
				...
				os: ["windows"],
				official: true,
				commands: ["winget install glab.glab"]
			},
			{
				manager: "scoop",
				os: ["windows"],
				official: false,
				commands: ["scoop install glab"]
			},
			{
				manager: "choco",
				os: ["windows"],
				official: false,
				commands: ["choco install glab"]
			},
			{
				manager: "dnf",
				os: ["fedora"],
				official: false,
				sudo: true,
				commands: ["sudo dnf install glab"]
			},
			{
				manager: "pacman",
				os: ["arch"],
			...
			},
			{
				...
				official: false,
				sudo: true,
				commands: ["sudo snap install glab"]
			},
			{
				manager: "nix",
				os: ["macos", "linux"],
				official: false,
				commands: ["nix profile install nixpkgs#glab"]
			},
			{
				manager: null,
				os: [
		...
		install: [
			{
				...
				os: ["macos", "linux"],
				official: false,
				commands: ["brew install tea"]
			},
			{
				manager: "scoop",
				os: ["windows"],
				official: false,
				commands: ["scoop install tea"]
			},
			{
				manager: "go",
				os: [
		...
		install: [
			{
				...
				os: ["macos", "linux"],
				official: false,
				commands: ["brew install forgejo-cli"]
			},
			{
				manager: "cargo",
				os: [
			...
			},
			{
				...
				when: (os) => os.distro?.id === "ubuntu" ? Number.parseFloat(os.distro.version ?? "0") >= 25.1 : os.distro?.id === "debian" && !os.distro.version
			},
			{
				manager: "nix",
				os: ["macos", "linux"],
...
function installOptions(spec, os, managers) {
	return spec.install.filter((r) => r.os.includes(os.family) || r.os.includes("linux") && os.platform === "linux").filter((r) => !r.manager || managers.includes(r.manager)).filter((r) => !r.when || r.when(os)).map((r) => {
		const out = {
		...
		if (r.note !== void 0) out.note = r.note;
		return out;
	});
}
function describeCli(host, os, managers, env) {
	...
	if (!spec.cli) {
		...
		if (spec.note !== void 0) result.note = spec.note;
		return result;
	}
	...
	if (path) {
		cli.version = run(path, spec.version ?? []).stdout.split("\n")[0] || null;
		const hostArg = host.selfHosted ? host.hostname ?? void 0 : void 0;
		const a = run(path, spec.auth?.(hostArg) ?? [], { env: {
			...env,
	...
	if (spec.login) cli.login = spec.login(host.selfHosted ? host.hostname ?? void 0 : void 0);
	...
	if (spec.docs !== void 0) cli.docs = spec.docs;
	return cli;
}
...
//#endregion
//#region src/env/hosts.ts
async function probe(hostname) {
	const get = async (path) => {
		try {
			const res = await fetch(`https://${hostname}${path}`, { signal: AbortSignal.timeout(5e3) });
			return {
				status: res.status,
				body: await res.text()
			};
...
async function collectHosts(dir, requested, doProbe) {
	...
	const add = (hostname, kind, remote) => {
		...
		const entry = byName.get(key) ?? {
			...
			kind,
			remotes: [],
			selfHosted: hostname ? !PUBLIC[hostname] : false
		};
		if (!entry.kind && kind) entry.kind = kind;
		if (remote && !entry.remotes.includes(remote)) entry.remotes.push(remote);
		byName.set(key, entry);
	};
	if (requested.length) for (const spec of requested) {
		const [kind, hostname] = spec.split("=");
		if (!kind || !KINDS.includes(kind)) usageError(`--host must be one of ${KINDS.join(", ")}`);
		add(hostname ?? Object.keys(PUBLIC).find((h) => PUBLIC[h] === kind) ?? null, kind);
	}
	else {
		const out = run("git", [
			"-C",
		...
		for (const line of out.split("\n")) {
			...
			add(hostname, (hostname ? PUBLIC[hostname] : void 0) ?? hostKind(hostname), name);
		}
	}
	const hosts = [...byName.values()];
	for (const h of hosts) if (doProbe && h.selfHosted && h.hostname) {
...
function mcpSources(dir, env) {
	...
	return [
		...
		},
		{
			harness: "copilot-cli",
			scope: "project",
			file: join(dir, ".copilot", "mcp-config.json"),
			key: "mcpServers"
		},
		{
			harness: "gemini",
		...
		},
		{
			harness: "gemini",
			scope: "project",
			file: join(dir, ".gemini", "settings.json"),
			key: "mcpServers"
		},
		{
			harness: "vscode",
			scope: "user",
			file: join(vscodeUserDir(env), "mcp.json"),
			key: "servers"
		},
		{
			harness: "vscode",
			scope: "project",
			file: join(dir, ".vscode", "mcp.json"),
			key: "servers"
		},
		{
			harness: "windsurf",
		...
		},
		{
			harness: "zed",
			scope: "project",
			file: join(dir, ".zed", "settings.json"),
			key: "context_servers"
		}
	];
}
...
function describeServer(name, entry) {
	const command = entry.command;
	const url = entry.url ?? entry.serverUrl ?? entry.httpUrl ?? (typeof command === "object" && !Array.isArray(command) ? command?.url : void 0);
	let origin = null;
	...
	const rawCommand = typeof command === "string" ? command : Array.isArray(command) ? command[0] : command?.path;
	const args = (Array.isArray(command) ? command.slice(1) : entry.args ?? (typeof command === "object" && !Array.isArray(command) ? command?.args : void 0) ?? []).filter((a) => typeof a === "string" && !a.includes("=") && !/token|key|secret|password|bearer/i.test(a));
	return {
		name,
...
function pluginSources(dir) {
	...
	const sources = [];
	for (const [id, installs] of Object.entries(installed)) for (const install of installs) {
	...
	for (const src of [...mcpSources(dir, env), ...pluginSources(dir)]) {
		...
		try {
			...
			if (src.toml) servers = tomlServers(text);
				...
				const flat = "flat" in src ? src.flat : false;
				const key = src.key ?? "";
				const blocks = [flat && !json[key] ? json : json[key]];
				if (src.project) {
					const projects = json["projects"];
					blocks.push(projects?.[src.project]?.[key]);
				}
				for (const [i, block] of blocks.entries()) for (const [name, entry] of Object.entries(block ?? {})) if (entry && typeof entry === "object") servers.push({
					...describeServer(name, entry),
					scope: i === 1 ? "local" : src.scope
				});
			}
		} catch (error) {
			found.push({
		...
		for (const s of servers) {
			...
			if (matches.length) found.push({
				harness: src.harness,
				file: src.file,
				...s,
				scope: s.scope ?? src.scope,
				disabled: s.disabled || Boolean("disabled" in src && src.disabled),
				hosts: matches
			});
		}
	}
...
async function detect({ dir, hosts: requested = [], probe: doProbe = false, env = process.env }) {
	...
	const kinds = [...new Set(hosts.map((h) => h.kind).filter((k) => k !== null))];
	return {
		dir,
		os,
		managers,
		hosts,
		mcp: collectMcp(dir, env, kinds)
	};
}

//#endregion
//#region src/skills/detect-env.ts
...
function printSummary(r) {
	const o = r.os;
	const lines = [`os: ${o.family}${o.distro?.name ? ` (${o.distro.name})` : o.version ? ` ${o.version}` : ""} ${o.arch}${o.wsl ? ", WSL" : ""}${o.sudo ? `, sudo: ${o.sudo}` : ""}`, `package managers: ${r.managers.join(", ") || "none found"}`];
	if (!r.hosts.length) lines.push("hosts: none (no git remotes; pass --host)");
	for (const h of r.hosts) {
		lines.push(`host: ${h.hostname ?? "?"} → ${h.kind ?? "unknown"}${h.selfHosted ? " (self-hosted)" : ""}${h.remotes.length ? ` [${h.remotes.join(", ")}]` : ""}`);
		const c = h.cli;
		if (!c) continue;
		if (!c.name) {
		...
		lines.push(c.installed ? `  cli: ${c.name} ${c.version ?? ""} — ${c.authenticated === true ? "authenticated" : c.authenticated === false ? `not authenticated (run: ${c.login})` : "auth unknown"}` : `  cli: ${c.name} not installed${c.install.length ? ", install options:" : " — no install recipe for this machine"}`);
		for (const i of c.install) {
			const how = i.commands.length ? i.commands.join(" && ") : i.note;
			lines.push(`    ${i.official ? "official" : "community"} via ${i.manager ?? "download"}: ${how}${i.commands.length && i.note ? ` (${i.note})` : ""}`);
		}
		if (!c.installed && c.postInstall?.length) lines.push(`    then: ${c.postInstall.join(" && ")}`);
		if (!c.installed) lines.push(`    docs: ${c.docs}`);
	}
	const mcp = r.mcp.filter((m) => !("error" in m));
	lines.push(`mcp servers for these hosts: ${mcp.length ? "" : "none found"}`);
	for (const m of mcp) lines.push(`  ${m.name} (${m.hosts.join(", ")}) — ${m.harness} ${m.scope}, ${m.url ?? [m.command, ...m.args].join(" ")}${m.disabled ? ", disabled" : ""}`);
	for (const m of r.mcp.filter((x) => "error" in x)) lines.push(`  ${m.file}: ${m.error}`);
